<?php
	...
	elseif ( get_pm($_GET['delete']) && !empty($_GET['delete']) && !empty($_GET['confirm']) && $_GET['confirm'] == 'true' ) {
		if (  delete_pm($_GET['delete']) ) {
			$alert['content'] = 'Great! Your workout was deleted.';
			$alert['type'] = 'success';
		}
		else {
	...
	//Here we establish the query to the database
		$query = 'WHERE id_user = '.$user['id_user'].' AND pm_type = "Workouts" ORDER BY pm_position ASC';

		//We verify if there is a pm in that section
		if( get_pm('count', $query) == 0 ) {
			$alert['content'] = 'There is no workout, you should add one.';
			$alert['type'] = 'info';
		}
        ...
        <div class="container-fluid">
            ...
            <div class="row">
                <div class="col-xl-12">
                    <div class="page-title-box">
                        <h4 class="page-title float-left">Workouts</h4>
            ...
            <!-- end row -->
						...
						<?php
							...
							small_desc('WORKOUT',
								'In this section you can manage your Workout Section and get notifications on the exact DAY and TIME that the workout is going to be performed. You will need to enable the Pushover app for the notifications, to set it up <a href="/setup.php?q=pushover">click here</a>'
							);
						?>
            ...
            <div class="row">
				...
				<div class="col-12">
						...
						<?php
							if ( !empty($_GET['view']) && get_pm($_GET['view'])['id_user'] == $user['id_user'] && get_pm($_GET['view'])['pm_type'] == 'Workouts' ) {

								$pm = get_pm($_GET['view']);
						?>

                        <div class="dropdown-menu dropdown-example pull-right " style="margin-bottom: 20px !important; margin-left: 20px !important;">
                            <a class="dropdown-item" href="/personal-workouts.php"><h6 class="dropdown-header">Go Back</h6></a>
                        </div>

                        <div class="dropdown-menu dropdown-example pull-right" style="margin-bottom: 20px !important;">
                            <a class="dropdown-item" href="/new-workout.php?edit=<?php echo $pm['id_pm']; ?>"><h6 class="dropdown-header">Edit Workout</h6></a>
                        </div>
				...
				<div class="col-12"><!-- THIS DIV ONLY WORK WHILE VIEW IS ACTIVE -->
                        ...
                        <div class="dropdown-menu dropdown-example pull-right" style="margin-bottom: 20px !important;">
                            <a class="dropdown-item" href="/new-workout.php"><h6 class="dropdown-header">New Workout</h6></a>
                        </div>
				</div>
				...
				<div class="col-12">
					...
					?>
                                        <tr plid="<?php echo $value['pm_position']; ?>" pmid="<?php echo $value['id_pm']; ?>">
                                            ...
                                            <td>
												<a href="/new-workout.php?edit=<?php echo $value['id_pm']; ?>" class="card-link"><i class="fa fa-edit"></i> </a>
					...
					?>
                        ...
                        <p class="text-muted m-b-15 font-13">
                            **<b>Click the name to see the routine.</b> You can move around the workouts, by dragging and dropping them. This will change the order in which they are displayed.
                        </p>
